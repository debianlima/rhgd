...
class TestRHGD(unittest.TestCase):
 ...
 def test_canonical_matcher_preserves_legacy_scheduler_symbol_without_runtime_ownership(self):
  nodes=[NodeCapability('edge',16384,.8,2,('code',))]
  units=[WorkUnit('w1','code',7000,1)]
  matcher=FederatedDestinationMatcher()
  self.assertEqual([(x.work_id,x.node_id) for x in matcher.match(units,nodes)],[('w1','edge')])
  self.assertIs(CognitiveScheduler,FederatedDestinationMatcher)
  self.assertFalse(any(hasattr(matcher,x) for x in ('queue','lease','admission','retry','runtime_state')))
